classdef Problem < handle
    properties
        p = [0.14,0.02,0.5] %model parameters
        p_train; %p_train represents the vector of sample points created after LHS design
        t0 = 0; %initial period of time
        Tend = 15; % end period of time
        y0 = [100 80]; %initial value of y
        points = 40; %total number of initial sampling points or ns
        ...
        change_var = 1; %total number of model parameters that are allowed to change 
        params_keys=[0 0 1];
        ...
        sampling_type='SLHS'; %'LHS' for Latin Hypercube sampling, 'SLHS' for Symmetric LHS and 'Corner' for corner points
        ...
        n=[0 0 2]; % number of points taken for interpolation of control function
...
%% METHODS FOR SIMPLE SURROGATE MODEL   
 ...
 % no mention of optimization
        ...
        function dy = ode_without_control(t,y,data)
            ...
            %function
            dy=zeros(2,1);
            dy(1)=data.p(3)*data.p(1)*y(1)-data.p(2)*y(1);
            dy(2)=(1-data.p(3))*data.p(1)*y(1)- data.p(2)*y(2);
        end
...
%              end
             ...
             dy=zeros(2,1);
            dy(1)=p(3)*p(1)*y(1)-p(2)*y(1);
            dy(2)=(1-p(3))*p(1)*y(1)- p(2)*y(2);
         end
         ...
         function psi0 = criteria(data)
             ...
             psi0 = -(trapz(t,0.5*y(:,1) + 0.5* y(:,2)));
             %-(trapz(t,0.5*y(:,1) + 0.5* y(:,2)));
         end
         ...
         function psi1 = constraint(data)
             ...
             psi1 =[abs(y(end,1) - data.yMax(1)); abs(y(end,2) - data.yMax(2))];
         end
